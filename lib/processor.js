        ...
        css.walkDecls(decl => {
            ...
            if (decl.value.indexOf('radial-gradient') !== -1) {
                if (OLD_RADIAL.test(decl.value)) {
                    ...
                    result.warn(
                        'Gradient has outdated direction syntax. ' +
                        'New syntax use `farthest-corner` instead of `cover`.',
                        { node: decl }
                    );
                } else if (/[^-]contain/.test(decl.value)) {
                    result.warn(
                        'Gradient has outdated direction syntax. ' +
                        'New syntax use `closest-side` instead of `contain`.',
                        { node: decl }
                    );
                }
            }
                ...
                if (decl.prop === 'grid-row') {
                    if (decl.value.indexOf('/') !== -1 &&
                        ...
                        result.warn(
                            ...
                            'You should add grid: false option ' +
                            'to Autoprefixer and use some JS grid polyfill ' +
                            'for full spec support',
                            { node: decl }
                        );
                    }
                }
