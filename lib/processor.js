...
class Processor {
  ...
  add (css, result) {
    ...
    function insideGrid (decl) {
      return decl.parent.some(subDecl => {
    ...
    let gridWasPrefixed = false

    css.walkDecls(decl => {
      if (this.disabledDecl(decl, result)) return undefined

      let gridPrefixes = this.gridEnabled(decl, result) &&
                       this.prefixes.add['grid-area'] &&
                       this.prefixes.add['grid-area'].prefixes

      if (gridPrefixes) {
        gridWasPrefixed = true
      }
      ...
      if (prop === 'grid-row-span') {
        ...
        if (value.indexOf('radial-gradient') !== -1) {
          if (OLD_RADIAL.test(decl.value)) {
            ...
            let match = value.match(RADIAL_BLOCK)
      ...
      if (
        ...
        if (
          display !== 'flex' && this.gridEnabled(decl, result) !== false
        ) {
  ...
  remove (css, result) {
    ...
    return css.walkDecls((decl, i) => {
      ...
      for (let checker of this.prefixes.values('remove', unprefixed)) {
        ...
        let notHack = this.prefixes.group(decl).down(
  ...
  disabledValue (node, result) {
    if (this.gridEnabled(node, result) === false && node.type === 'decl') {
  ...
  disabledDecl (node, result) {
    if (this.gridEnabled(node, result) === false && node.type === 'decl') {
      if (
        node.prop.indexOf('grid') !== -1 || node.prop === 'justify-items'
      ) {
        return true
      }
    }
    if (this.prefixes.options.flexbox === false && node.type === 'decl') {
      ...
      if (
        node.prop.indexOf('flex') !== -1 || other.indexOf(node.prop) !== -1
      ) {
        return true
      }
    }
  ...
  /**
   * Check for control comment on "autoprefixer grid: on"
   */
  gridEnabled (node, result) {
    if (!node) return false
    ...
    if (node.nodes) {
      ...
      node.each(i => {
        if (i.type !== 'comment') return
        if (/(!\s*)?autoprefixer grid:\s*(off|on)/i.test(i.text)) {
          if (typeof status !== 'undefined') {
            result.warn(
              'Second Autoprefixer control comment ' +
              'was ignored. Autoprefixer applies control ' +
            ...
            status = /(!\s*)?autoprefixer grid:\s*(on)/i.test(i.text)
          }
        }
      })
    ...
    if (!node.nodes || value === null) {
      if (node.parent) {
        let isParentDisabled = !this.gridEnabled(node.parent, result)
        if (isParentDisabled === true) {
          value = false
        }
      }
    }
    ...
    if (value === null) {
      return this.prefixes.options.grid
    } else {
      return value
    }
  }
