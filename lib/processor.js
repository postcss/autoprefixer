...
class Processor {
    ...
    add(css, result) {
        ...
        // Selectors
        css.walkRules(rule => {
            if (this.disabledForAdd(rule)) return undefined;
        ...
        css.walkDecls(decl => {
            if (this.disabledForAdd(decl)) return undefined;
        ...
        return css.walkDecls(decl => {
            if (this.disabledForAdd(decl)) return;
    ...
    remove(css) {
        ...
        css.walkAtRules((rule, i) => {
            if (this.prefixes.remove[`@${rule.name}`]) {
                if (!this.disabledForRemove(rule)) {
        ...
        return css.walkDecls((decl, i) => {
            if (this.disabledForRemove(decl)) return;
