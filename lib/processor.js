...
const GRID_REGEX = /(!\s*)?autoprefixer\s*grid(\s*autoplace)?:\s*(off|on)/i
    ...
    if (node.nodes) {
      ...
      node.each(i => {
        ...
        if (GRID_REGEX.test(i.text)) {
          if (typeof status !== 'undefined') {
            result.warn(
              'Second Autoprefixer grid control comment ' +
              'was ignored. Autoprefixer applies control ' +
              'comment to whole block, not to next rules.',
              { node: i }
            )
          } else if (/autoplace/i.test(i.text) && /on/i.test(i.text)) {
            status = 'autoplace'
          } else if (/autoplace/i.test(i.text) && /off/i.test(i.text)) {
            status = true
          } else {
            status = /on/i.test(i.text)
          }
