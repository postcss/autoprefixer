  ...
  add (css, result) {
    ...
    css.walkDecls(decl => {
      ...
      if (prop === 'grid-row-span') {
        ...
        result.warn(
          'grid-column-span is not part of final Grid Layout. ' +
                    'Use grid-column.',
          { node: decl }
        )
        return undefined
        ...
        return undefined
      } else if (prop === 'display' && /(inline-)?grid/.test(value)) {
        let testNeighbourItem = i => /(align|justify|place)-items/.test(i) &&
          this.prefixes.unprefixed(i.prop) === this.prefixes.normalize(i.prop)

        let testNeighbourContent = i =>
          /(align|justify|place)-content/.test(i) &&
          this.prefixes.unprefixed(i.prop) === this.prefixes.normalize(i.prop)

        decl.parent.some(neigh => {
          if (testNeighbourItem(neigh)) {
            let selfEquiv = neigh.prop.replace('-items', '-self')
            result.warn('IE does not support align-items, justify-items, ' +
              ...
              'instead. ' +
              `Example: .grid > * { ${ selfEquiv }: ${ neigh.value } }`,
            { node: neigh })
          }

          if (testNeighbourContent(neigh)) {
            result.warn(
    ...
    if (this.prefixes.options.grid === true) {
