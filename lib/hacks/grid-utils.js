...
function isSameNextRule(rule, selector) {
    if (
        rule &&
        rule.type === 'atrule' &&
        rule.name === 'media' &&
        rule.nodes.some(o => o.selector === selector)
    ) {
        return true;
    }
    return false;
...
function insertAreas(areas, decl, result) {
    ...
    decl.root().walkDecls('grid-area', gridArea => {
        ...
        if (area && parentMedia) {
            ...
            const root = parentMedia.parent;
            const parentMediaIdx = root.index(parentMedia);
            const gridAreaIdx = root.index(gridArea.parent);
            const nextRuleIdx = root.nodes[gridAreaIdx + 1];
            const isSameRule = isSameNextRule(nextRuleIdx, rule.selector);

            if (gridAreaIdx > parentMediaIdx && !isSameRule) {
                const newMedia = parentMedia.clone().removeAll();
                newMedia.append(rule);
                gridArea.parent.after(newMedia);
            } else if (isSameRule) {
                return undefined;
            }  else {
