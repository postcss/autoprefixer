...
/**
 * change selectors for duplicate areas
...
function insertAreas (areas, decl, result) {
  ...
  let rulesOutsideMedia = []
  let rulesInsideMedia = []
  let areasLength = Object.keys(areas).length
  let outsideMediaAreasCount = 0
  let insideMediaAreasCount = 0
  let duplicates = getDuplicateAreaNames(decl)
  ...
  decl.root().walkDecls('grid-area', gridArea => {
    ...
    if (parentMedia) {
      ...
      rulesInsideMedia.push(rule)
      insideMediaAreasCount++

      // if we found all needed area INSIDE media
      if (insideMediaAreasCount === areasLength) {
        let next = gridArea.parent.next()

        if (duplicates.length > 0) {
          let selectors = decl.parent.selectors
          rulesInsideMedia = (
            changeDuplicateAreaSelectors(rulesInsideMedia, selectors)
          )
        }
        ...
        let selectors = rulesInsideMedia.map(r => r.selector)
        let lastSimilarMedia = getLastNextSimilarMedia(next, selectors)

        let areaParentMedia = getParentMedia(gridArea.parent)
        let areaMedia = parentMedia.clone().removeAll().append(rulesInsideMedia)
      ...
      rulesOutsideMedia.push(rule)
      outsideMediaAreasCount++

      // if we found all needed areas OUTSIDE media
      if (outsideMediaAreasCount === areasLength && duplicates.length > 0) {
        let selectors = decl.parent.selectors
        rulesOutsideMedia = (
          changeDuplicateAreaSelectors(rulesOutsideMedia, selectors)
        )
        gridArea.parent.after(rulesOutsideMedia)
        return false
      }
    }
