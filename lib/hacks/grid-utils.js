...
// TODO: this function bugs when used repeat function and grid-gap
...
// TODO: add js comment about the function
function autoplaceGridItems (decl) {
  let { parent } = decl
  let rowDecl = parent.nodes.find(i => i.prop === 'grid-template-rows')

  if (!rowDecl) {
    // TODO: warn user that autoplacement won't work
    return undefined
  }
  ...
  let filledRows = rows.map((_, rowIndex) => {
    return Array.from({ length: columns.length }, (v, k) =>
      k + (rowIndex * columns.length)).join(' ')
  })

  let areas = parseGridAreas({ rows: filledRows, gap })

  Object.keys(areas).reverse().forEach(area => {
    let { column, row } = areas[area]

    let nodeSelector = parent.selectors.map(sel =>
      sel + ` > *:nth-child(${ parseInt(area) + 1 })`).join(', ')
