...
function getDuplicateAreaNames (decl) {
  ...
  root.walkDecls(/grid-template(-areas)?$/g, d => {
    ...
    if (inMediaRule) {
      let selectorIsFound = nodeRule.selectors.some(sel => {
...
function shouldAddSpan (decl, area, areaName) {
  ...
  root.walkRules(rule.selector, node => {
    ...
    node.walkDecls(/grid-template(-areas)?$/, d => {
...
function changeDuplicateAreaSelectors (rules, templateSelectors) {
  rules = rules.map(rule => {
    let selectorBySpace = list.space(rule.selector)
    let selectorByComma = list.comma(rule.selector)

    if (selectorBySpace.length > selectorByComma.length) {
      rule.selector = selectorBySpace.slice(-1).join('')
    }
    return rule
  })
...
/**
 * Run over every grid-area and prefix declarations.
 * If grid-template is inside media - pack grid-area rules and insert
 * them into media rule.
 * @param  {Object} areas
 * @param  {Declaration} decl
 * @param  {Result} result
 * @return {void}
...
function insertAreas (areas, decl, result) {
  ...
  root.walkDecls('grid-area', gridArea => {
    let next = gridArea.parent.next() || gridArea.parent
    let areaParentMedia = getParentMedia(gridArea.parent)
    let value = gridArea.value
    let area = areas[value]
    let gridAreaSelector = gridArea.parent.selector
    let selectorBySpace = list.space(gridAreaSelector)
    let selectorByComma = list.comma(gridAreaSelector)

    let selectorIsComplex = (
      selectorBySpace.length > 2 && selectorByComma.length === 1
    )
    ...
    // skip if rule has no area we need
    if (!area) {
      return true
    }

    // if we are inside media rule
    if (parentMedia) {
      ...
      // if we found all needed areas
      if (areasCount === areasLength) {
        let selectors = rules.map(r => r.selector)
        let lastSimilarRule = getLastSimilarRule(next, selectors)

        // if we found duplicates inside grid-template with the same selector
        if (selectorIsComplex || (hasDuplicates && !sameSelector)) {
          // change selectors for rules
          rules = changeDuplicateAreaSelectors(rules, templateSelectors)
        }
        ...
        // prevent doubling media duplicate rules with this condition
        let duplicateMedia = (
          lastSimilarRule &&
          ...
          rules[0].selector.toString()) &&
          /^-ms-/.test(lastSimilarRule.first.first.prop)
        )

        // abort if media are duplicate.
        // We don't want to add the same rules twice
        if (duplicateMedia) {
          return false
        }
        ...
        if (areaParentMedia) {
          // insert after grid-area @media
          areaParentMedia.after(areaMedia)
        } else if (root.index(parentMedia) > root.index(gridArea.parent)) {
          // insert INSIDE grid-template @media
          parentMedia.append(rules)
        } else if (
          lastSimilarRule &&
          lastSimilarRule.name === 'media' &&
          lastSimilarRule.params === parentMedia.params
        ) {
          // insert INSIDE the closest similar rule
          lastSimilarRule.append(rules)
        } else {
          // insert after the closest similar rule with the same selectors
      ...
      // prefix decls ONLY if grid-template has no duplicates
      if (!hasDuplicates) {
        gridArea.parent.walkDecls(/-ms-grid-(row|column)/, d => {
          d.remove()
        })
      ...
      // add rule to the array. Push only if it's inside media
      if (!areaParentMedia) {
        rules.push(rule)
        areasCount++
      }

      // if we found all areas and have duplicates
      if ((areasLength === areasCount) && hasDuplicates) {
        // not duplicate rules if grid-area selector is complex
        if (selectorIsComplex) {
          return false
        }

        let selectors = rules.map(r => r.selector)
        let lastSimilarRule = getLastSimilarRule(gridArea.parent, selectors)

        rules = changeDuplicateAreaSelectors(rules, templateSelectors)

        // prevent doubling media rules with this condition
        let duplicateMedia = (
          lastSimilarRule && lastSimilarRule.type === 'atrule' &&
          lastSimilarRule.name === 'media' &&
          lastSimilarRule.first.type === 'rule' &&
          (lastSimilarRule.first.selector.toString() ===
          rules[0].selector.toString()) &&
        ...
        let duplicateRule = (
          ...
          rules[lastIndex].selector.toString()) &&
          /^-ms-/.test(lastSimilarRule.first.prop)
        )

        // abort if rules are duplicate. We don't want to add the same rules
        if (duplicateMedia || duplicateRule) {
          return false
        }
   ...
   * (only if duplicated found in the different selector)
   */
