...
function insertGridGap (decl, gap) {
  let rule = decl.parent
  ...
  root.walkRules(rule.selector, r => {
    ...
    // find grid-gap value
    r.walkDecls('grid-gap', d => (gridGap = d.value))
  ...
  // if we find the closest rule
  if (closestRule) {
    let gapDecl
    // get grid-gap declaration
    closestRule.walkDecls('grid-gap', d => (gapDecl = d.clone()))
    // and paste in our rule (before grid-template decl)
    rule.walkDecls(/grid-template(-areas)?$/g, d => {
      gapDecl.cleanRaws()
      d.before(gapDecl)
      return undefined
    })
