...
function inheritGridGap (decl, gap) {
  ...
  let root = rule.root()
  let selectorBySpace = list.space(rule.selectors[0])
  ...
  let selector = selectorBySpace[selectorBySpace.length - 1]
    .split(/\.|#/)
    .slice(1)[0]
  let escaped = escapeRegexp(rule.selector)

  let regexp = new RegExp(
    `([#.]${ selector }$)|([#.]${ selector },)|(${ escaped }$)|(${ escaped },)`
  )
  ...
  root.walkRules(regexp, r => {
    ...
    // skip nodes that have higher specifity
    let nodeSelectorBySpace = list.space(r.selectors[0])
    if (selectorBySpace.length < nodeSelectorBySpace.length) {
      return true
    }
