...
function getLastSimilarRule (rule, selectors, ruleToCompare) {
  ...
  if (type === 'rule') {
    ...
    let selectorsEqual = selectors.every((sel, index) => {
      return (
        selectorsToCompare[index] && selectorsToCompare[index].includes(sel)
      )
    })
  ...
  root.walkDecls('grid-area', gridArea => {
    ...
    if (parentMedia) {
      ...
      if (areasCount === areasLength) {
        ...
        let duplicateRules = rules.some(r => {
          let duplicateMedia = (
            lastSimilarRule &&
        ...
        } else if (next && next.params === parentMedia.params) {
          next.append(rules)
        } else {
