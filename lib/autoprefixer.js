...
// Some helpers for `autoprefixer.prefixer`.
var prefixer = {
    // Add prefixes to properties in translation.
    transition: function(value, prop, vendor) {
        return value.replace(prop.regexp,
            function(all, before, el, params, _, after) {
                ...
                var prefixes = [''];
                if ( !vendor ) {
                    prefixes = prefixes.concat(prop.prefixes);
                } else {
    ...
    // Common code to add prefixes to values
    value: function (vendor, rules, rule, num, prop) {
        if ( !prop ) {
            return num;
        }

        var value, name;
        for ( name in prop.values ) {
            ...
            value.prefixes.forEach(function (prefix) {
                ...
                rules.splice(num, 0, {
                    ...
                    value:    rule.value.replace(value.regexp,
                              '$1' + prefix + name + '$2')
                });
                num += 1;
            });
...
var autoprefixer = {
    ...
    prefixer: function (props, style, css) {
        var vendor, prop, num, rule, i, split, prefix;
        ...
        rework.visit.declarations(style, function (rules, node) {
            ...
            for ( num = 0; num < rules.length; num += 1 ) {
                ...
                split  = prefixer.splitPrefix(rule.property);
                prop   = props[split.name];
                prefix = split.prefix;

                num = prefixer.value(prefix || vendor, rules, rule, num, all);
                num = prefixer.value(prefix || vendor, rules, rule, num, prop);
                ...
                for ( i in transitions ) {
                    prop = transitions[i];
                    if ( prefix ) {
                        var contain = prop.prefixes.some(function (i) {
                            return i == prefix;
                        });
                        if ( !contain ) {
                            continue;
                    ...
                    rule.value = prefixer.transition(rule.value, prop, prefix);
                }
            }
        });
    },
