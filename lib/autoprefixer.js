...
var autoprefixer = {
    ...
    prefixer: function (props, style, css) {
        var vendor, keyframes, prop, num, rule;
        ...
        rework.visit.declarations(style, function (rules, node) {
            ...
            for ( num = 0; num < rules.length; num += 1 ) {
                ...
                if ( prop && prop.prefixes ) {
                    ...
                    num = value(keyframes, vendor, rules, rule, num, all);
                    num = value(keyframes, vendor, rules, rule, num, prop);
                }
            };
        });
    },

    valuePrefixer: function (keyframes, vendor, rules, rule, num, prop) {
        if ( !prop ) {
            return num;
        }
