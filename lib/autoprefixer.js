...
var autoprefixer = {
    ...
    rework: function (requirements) {
        ...
        return function (style, css) {
            autoprefixer.unprefixer(unprefixes, style, css);
            autoprefixer.prefixer(prefixes, style, css);
        };
    },
    ...
    unprefixer: function (remove, style, css) {
        ...
        rework.visit.declarations(style, function (rules, node) {
            for ( var num = 0; num < rules.length; num += 1 ) {
                ...
                if ( prop === 'transition' || prop === 'transition-property' ) {
                    values = values.concat(remove.values._transition);
                }
    ...
    filter: function (data, browsers, exclude) {
        var selected = { };
    ...
    unprefixes: function (props, values) {
        var removeProps  = { };
        var removeValues = { _transition: [] };

        for ( var name in this.data.props ) {
            var prefixes = this.data.props[name].browsers.map(function (b) {
                var key = b.split(' ')[0];
                return autoprefixer.data.browsers[key].prefix;
            });
            uniq(prefixes).filter(function (prefix) {
                ...
                if ( autoprefixer.data.props[name].transition ) {
                    removeValues._transition.push(containRegexp(prefix + name));
                }
                removeProps[prefix + name] = true;
            })
        }

        for ( var name in this.data.values ) {
            var value = this.data.values[name];
            var prefixes = value.browsers.map(function (b) {
                var key = b.split(' ')[0];
                return autoprefixer.data.browsers[key].prefix;
            });
            var names = uniq(prefixes).filter(function (prefix) {
                if ( !values[name] ) {
                    return true;
                }
            ...
            value.props.forEach(function (prop) {
                if ( !removeValues[prop] ) {
                    removeValues[prop] = [];
                }
                removeValues[prop] = removeValues[prop].concat(names);
            });
        }

        return { props: removeProps, values: removeValues };
    }
};
