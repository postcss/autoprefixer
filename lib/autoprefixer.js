...
var autoprefixer = {
    ...
    prefixer: function (props, style, css) {
        var vendor, prop, num, rule, transed;
        ...
        var transeds = { };
        for ( prop in props ) {
            if ( props[prop].transition && props[prop].prefixes ) {
                transeds[prop] = props[prop];
            }
        }

        var all   = props['*'];
        var value = autoprefixer.valuePrefixer;
        rework.visit.declarations(style, function (rules, node) {
            ...
            for ( num = 0; num < rules.length; num += 1 ) {
                ...
                num = value(vendor, rules, rule, num, all);
                num = value(vendor, rules, rule, num, prop);
                ...
                for ( prop in transeds ) {
                    transed    = transeds[prop];
                    rule.value = rule.value.replace(
                        transed.regexp,
                        function(all, before, el, _, after) {
                            return before + el + ', ' +
                                   transed.prefixes.map(function(prefix) {
                                        return prefix + el;
                                   }).join(', ') +
                                   after;
                        });
    ...
    valuePrefixer: function (vendor, rules, rule, num, prop) {
        if ( !prop ) {
            return num;
        }
