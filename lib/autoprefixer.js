...
var prefixer = {
    ...
    transition: function(value, prop) {
        return value.replace(prop.regexp,
            function(all, before, el, params, _, after) {
                if ( typeof(params) == 'undefined' ) {
                    params = ''
                }
                return before + el + params + ', ' +
                       prop.prefixes.map(function(prefix) {
    ...
    prefixer: function (props, style, css) {
        ...
        rework.visit.declarations(style, function (rules, node) {
            ...
            for ( num = 0; num < rules.length; num += 1 ) {
                ...
                num = prefixer.value(vendor, rules, rule, num, all);
                num = prefixer.value(vendor, rules, rule, num, prop);

                if ( rule.property != 'transition' &&
                     rule.property != 'transition-property' ) {
                    continue;
                }

                for ( i in transitions ) {
                    prop = transitions[i];
                    rule.value = prefixer.transition(rule.value, prop);
                }
            }
